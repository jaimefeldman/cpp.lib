...
.PHONY: info clean project debug memchk test gdb
.SILENT: clean debug memchk test gdb $(APP) $(OBJECTS_FILES) $(OBJSUBDIRS) $(C_OBJECTS_FILES) 
...
ifeq (memchk, $(CMDGOAL))
	...
	CCFLAGAS := -ggdb -std=c++17 -Wall -Wno-c++11-extensions 
	xx:=$(shell if [ $(UNAME) == "Darwin" ]; then \
...
else ifeq (gdb, $(CMDGOAL))

	xx:=$(shell if [ $(UNAME) == "Darwin" ]; then \
		echo -e "[ ${yellowdark}Compilando para depuración con ${purple}gdb (${yellow}$(PROJECT_NAME)${yellowdark}) - ${blue}MacOS${NOC} ] ${gris}${FECHA_HOY}"; \
	elif [ $(UNAME) == "Linux" ]; then \
		echo -e "[ ${yellowdark}Limpiando (${yellow}$(PROJECT_NAME)${yellowdark}) - ${greenDark}Linux${NOC} ] ${gris}${FECHA_HOY}"; \
	fi)
